...
- **左サイドバー**: プロジェクト新規/開く、折りたたみ式「ツール」メニュー（PNG 出力・全ページ一括 PNG・セリフ TXT・参照キャラクター・背景ライブラリ・背景＋人物合成）、モザイク操作、**ページ一覧**（追加・並べ替え・削除、メイン領域）、Assets 整理
  - ツールメニューはデフォルト閉じ。展開すると 6 つのボタンが縦に並ぶ。ページ一覧が常に大きく表示されるよう優先レイアウト
